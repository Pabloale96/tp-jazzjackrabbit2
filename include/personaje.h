...
protected:
    Renderer& renderer;
    Surface spaz{Surface(
            IMG_PATH
            "/spaz.png")};  // inicializo cualquier cosa en total se pisa a crear un personaje
    Texture sprites{Texture(
            renderer,
            spaz.SetColorKey(true, SDL_MapRGB(spaz.Get()->format, 44, 102,
                                              150)))};  // inicializo cualquier cosa en total se
                                                        // pisa a crear un personaje
    // Animacion movimiento{Animacion(std::ref(renderer),std::ref(sprites))};//inicializo cualquier
    // cosa en total se pisa a crear un personaje
public:
    explicit Personaje(Renderer& renderer): renderer(renderer) {}
    ~Personaje() {}
...
private:
    ...
    Texture sprites{Texture(renderer,
    ...
    AnimacionJumpAndMove aniJumpAndMove{
            AnimacionJumpAndMove(renderer, sprites, frames_jump_and_move)};
    AnimacionIntoxicated aniIntoxicated{
            AnimacionIntoxicated(renderer, sprites, frames_intoxicated)};
    ...
    AnimacionShootAndMove aniShootAndMove{
            AnimacionShootAndMove(renderer, sprites, frames_shoot_and_move)};
    AnimacionStopShootAndMove aniStopShootAndMove{
            AnimacionStopShootAndMove(renderer, sprites, frames_stop_shoot_up_move)};
    AnimacionShootAndFalling aniShootAndFalling{
            AnimacionShootAndFalling(renderer, sprites, frames_shoot_and_falling)};
    AnimacionStopShootAndFalling aniStopShootAndFalling{
            AnimacionStopShootAndFalling(renderer, sprites, frames_stop_shoot_and_falling)};
