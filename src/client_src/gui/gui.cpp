...
Gui::Gui(Queue<msgAccion>& client_commands, std::shared_ptr<PersonajeGui>& jugador,
        ...
        jugador(jugador),
        gamestate(gamestate),
        escenario(Escenario(gamestate->obtener_vector_plataforma())) {}
...
bool Gui::run(int h_window, int w_window) {
    ...
    for (auto& [id, personaje]: gamestate->obtener_diccionario_de_personajes()) {        
        float x = (personaje.obtener_posicion_x() - jugador->obtener_posicion_x()) *SCALING_VALUE_PIXEL_X + h_window/2; 
        float y = (personaje.obtener_posicion_y() - jugador->obtener_posicion_y()) *SCALING_VALUE_PIXEL_Y + w_window/2; 
        personaje.setPosicion(x,y);
        if (x <= (jugador->obtener_posicion_x() + h_window/2) || x <= (jugador->obtener_posicion_x() - h_window/2) || y <=
            (jugador->obtener_posicion_y() + w_window/2) || y <= (jugador->obtener_posicion_y() - w_window/2)) { 
