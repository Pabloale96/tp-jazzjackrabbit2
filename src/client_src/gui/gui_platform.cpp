...
void PlatformGui::setFrame(std::shared_ptr<Frame>& frame_plat, int width_window, int height_window) {
    platform=frame_plat;
...
void PlatformGui::show(int dif_x, int dif_y, int i) {
    ...
    if (this->checkIsInWindow(dif_x* SCALING_VALUE_PIXEL_X, dif_y* SCALING_VALUE_PIXEL_Y)) {
        ...
        if (rotate == 0) {
            ...
            platform->copy(rotate, posicion_x-height_sc/2, posicion_y_invertida+width_sc/2, height_sc,width_sc );
        } else {
